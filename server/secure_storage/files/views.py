...
def signup(request):
    ...
    if serializer.is_valid(): #validate data
        serializer.save() #if valid creates account
...
def login(request):
    ...
    if authenticate_user is not None:
        ...
        #if exist store in token, if not create and store in created_token.
...
#needs to have a token to access this request.
@api_view(["GET"])
@authentication_classes([SessionAuthentication, TokenAuthentication])
@permission_classes([IsAuthenticated]) #must be authenticated with token authentication
